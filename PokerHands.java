  ...
  public void calculate() {
    ...
    if (this.allSameSuit() != -1 && this.straight() != -1) {
      if (this.getCard(0).getValue() == 10) {
        this.category = PokerHands.ROYAL_FLUSH;
        this.handValue = 9999;
        return;
      } else {
        this.category = PokerHands.STRAIGHT_FLUSH;
        return;
      }
    }

    if (this.fours() != -1) {
      this.category = PokerHands.FOUR_OF_A_KIND;
      return;
    }

    if (this.fullHouse() != -1) {
      this.category = PokerHands.FULL_HOUSE;
      return;
    }

    if (this.allSameSuit() != -1) {
      this.category = PokerHands.FLUSH;
      return;
    }

    if (this.straight() != -1) {
      this.category = PokerHands.STRAIGHT;
      return;
    }

    if (this.triple() != -1) {
      this.category = PokerHands.THREE_OF_A_KIND;
      return;
    }

    if (this.twoPairs() != -1) {
      this.category = PokerHands.TWO_PAIRS;
      return;
    }

    if (this.pair() != -1) {
      this.category = PokerHands.ONE_PAIR;
      return;
    }

    this.handValue = this.getCard(4).getValue();
    this.category = PokerHands.HIGH_CARD;
  }
