...
Poker Game Implementation Test file With Sample Test Cases given in a Text file
Expected Result : Player 1: 263 hands
                  Player 2: 237 hands
Required File in the Same folder as this Class and the FileName is poker-hands.txt
Ramyaa Madhan
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      while (input != null) {
        ...
        if (!input.matches("(?:[2-9TJQKAtqjka][SCHDschd] ){9}[2-9TJQKAtqjka][SCHDschd]")) {
  ...
  public void calculate() {
    ...
    if (this.allSameSuit() != -1 && this.straight() != -1) {
      if (this.getCard(0).getValue() == 10) {
        this.category = PokerHandsTest.ROYAL_FLUSH;
        this.handValue = 9999;
        return;
      } else {
        this.category = PokerHandsTest.STRAIGHT_FLUSH;
        return;
      }
    }

    if (this.fours() != -1) {
      this.category = PokerHandsTest.FOUR_OF_A_KIND;
      return;
    }

    if (this.fullHouse() != -1) {
      this.category = PokerHandsTest.FULL_HOUSE;
      return;
    }

    if (this.allSameSuit() != -1) {
      this.category = PokerHandsTest.FLUSH;
      return;
    }

    if (this.straight() != -1) {
      this.category = PokerHandsTest.STRAIGHT;
      return;
    }

    if (this.triple() != -1) {
      this.category = PokerHandsTest.THREE_OF_A_KIND;
      return;
    }

    if (this.twoPairs() != -1) {
      this.category = PokerHandsTest.TWO_PAIRS;
      return;
    }

    if (this.pair() != -1) {
      this.category = PokerHandsTest.ONE_PAIR;
      return;
    }

    this.handValue = this.getCard(4).getValue();
    this.category = PokerHandsTest.HIGH_CARD;
  }
